const nokosButton = document.getElementById('nokosButton');
        ...
        const nokosPrice = 10000; // Harga per Nokos
        const nokosQuantitySection = document.getElementById('nokosQuantitySection');
        const nokosQuantityInput = document.getElementById('nokosQuantity');
        const totalNokosPriceSpan = document.getElementById('totalNokosPrice');
        const beliNokosSekarangButton = document.getElementById('beliNokosSekarang');
        let selectedNokosCountry = '';

        countrySelect.addEventListener('change', () => {
            selectedNokosCountry = countrySelect.value;
            if (selectedNokosCountry) {
                nokosDetails.style.display = 'block';
                nokosQuantitySection.style.display = 'block'; // Tampilkan bagian input jumlah
                ...
                nokosDetails.style.display = 'none';
                nokosQuantitySection.style.display = 'none'; // Sembunyikan jika negara tidak dipilih
                nokosPriceElement.textContent = 'Harga akan muncul setelah memilih negara';
            }
            updateTotalNokosPrice(); // Perbarui total harga saat negara dipilih (default 1)
        });

        nokosQuantityInput.addEventListener('input', () => {
            updateTotalNokosPrice();
        });

        function updateTotalNokosPrice() {
            const quantity = parseInt(nokosQuantityInput.value);
            if (!isNaN(quantity) && quantity > 0) {
                const totalPrice = quantity * nokosPrice;
                totalNokosPriceSpan.textContent = `Rp ${totalPrice.toLocaleString('id-ID')}`;
            } else {
                totalNokosPriceSpan.textContent = `Rp ${nokosPrice.toLocaleString('id-ID')}`; // Harga default 1 nokos
                nokosQuantityInput.value = 1; // Pastikan nilai minimal 1
            }
        }

        beliNokosSekarangButton.addEventListener('click', (event) => {
            ...
            const quantity = nokosQuantityInput.value.trim();
            const service = document.getElementById('service').value;
            const totalPrice = totalNokosPriceSpan.textContent;
            const premiumEmojis = '💎✨👑';

            const telegramMessage = `🔥 *PESANAN NOKOS* 🔥 ⭐ ${premiumEmojis}\n\n🌍 *Negara:* ${selectedNokosCountry.toLowerCase()}\n🔢 *Jumlah:* ${quantity}\n✉️ *Layanan:* ${service}\n💰 *Total Harga:* ${totalPrice}\n\nTerima kasih telah order di Ravaelstore! 🙏`;
